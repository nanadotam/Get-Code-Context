...
# This script works for Python projects
# Place this script in the root folder of your project
# Run the command chmod +x get_code_context_python.sh
# Then run ./get_code_context_python.sh
...
directories=("." "src" "notebooks" "lib")

# List of file types to ignore
ignore_files=("*.pyc" "*.pyo" "*.png" "*.jpg" "*.jpeg" "*.gif" "*.svg")
...
read_files() {
    for entry in "$1"/*; do
        if [ -d "$entry" ]; then
            ...
            # If the file type should not be ignored, check for .py or .ipynb files and append content to the output file
            if ! $should_ignore && [[ "$entry" == *.py || "$entry" == *.ipynb ]]; then
